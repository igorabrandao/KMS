<?php
	class CadastrarAparelhoModel extends MainModel
	{
		/**
		 * Class constructor
		 *
		...
		*/
		public function insert_device()
		{
			...
			/**
			 * Check if information was sent from web form with a field called insere_aparelho.
			*/
			if ( 'POST' != $_SERVER['REQUEST_METHOD'] || empty( $_POST['insere_aparelho'] ) )
			{
				return;
			}
			...
			*/
			...
			// Remove insere_aparelho field to avoid problems with PDO
			unset($_POST['insere_aparelho']);
			unset($_POST['N_APARELHOS']);

			// Insert contract's head in database
			$query = $this->db->insert( 'patrimonio.loteaparelho', array_slice($_POST, 0, 1) );

			// Check operation
			if ( $query )
			{
				// Add last inserted ID to $_POST array
				$_POST['ID_LOTE_APARELHO'] = $this->db->last_id;

				// Split IMEI array
				$IMEI_list = explode("//", $_POST['elem_IMEI']);
				unset($_POST['elem_IMEI']);

				// Insert multiple registers in DB
				for ( $i = 0; $i < sizeof($IMEI_list); $i++ )
				{
					// Update current IMEI
					$_POST['IMEI'] = $IMEI_list[$i];

					// Insert contract's terms
					$query2 = $this->db->insert( 'patrimonio.patrimonio', array_slice($_POST, 1) );
					...
					if ( $query2 )
					{
						...
						if ( ! empty($_FILES['file_anexo']['name']) )
						{
							...
							$contract_path = ABSPATH . '/resources/patrimonio/aparelho/' . $_POST['ID_LOTE_APARELHO'];
								...
								if ( $_FILES['file_anexo']['error'][$key] == 0 )
								{
									...
									else
									{
										...
										if( move_uploaded_file($_FILES["file_anexo"]["tmp_name"][$key], $contract_path . "/" . $_FILES['file_anexo']['name'][$key]) )
										{
											...
											$arr_data['ID_LOTE_APARELHO'] = $_POST['ID_LOTE_APARELHO'];
											$arr_data['ANEXO'] = $contract_path . "/" . $_FILES['file_anexo']['name'][$key];
											$query3 = $this->db->insert( 'patrimonio.loteaparelhoanexo', $arr_data );
										}
				...
				// Return a message
				$this->form_msg = '<p class="success">Aparelho(s) cadastrado com sucesso!</p>';

				// Redirect
				?><script>alert("Aparelho(s) cadastrado com sucesso!"); window.location.href = "<?php echo HOME_URI ?>";</script> <?php

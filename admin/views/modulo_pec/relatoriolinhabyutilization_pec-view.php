										...
										<table class="table table-striped table-bordered table-hover" id="sample_1">
											...
											<tbody>
												<?php
													...
													$data_value = $modelo->detailedutilizationreportgrouped_PEC($modelo->getIdPEC(), $utilization_id);
													...
													foreach ( $data_value as $value )
													{
														...
														// Service description
														echo "<td><a href='" . encrypted_url($modelo->getIdPEC(), HOME_URI . "/modulo_pec/relatoriolinha_detalhamento_pec?idPEC=") . 
															encrypted_url($utilization_id, "&idUtilizacao=") . encrypted_url($value["ID_PEC_TIPO_LIGACAO"], "&idTipoLigacao=") . "' title='" . $value["DESC_LIGACAO"] . "'>" . 
															$value["DESC_LIGACAO"];
														...
														switch ( $value["ID_TIPO_DET"] )
														{
															...
															// ---------------------------------------------------------------------------
																...
																// Duration
																$aux_duration_value = "";
																$duration_list = $modelo->get_duration_list($modelo->getIdPEC(), iif($value["ID_PEC_TIPO_LIGACAO"]), "", iif($value["ID_TIPO_UTILIZACAO"]));
															...
															// ---------------------------------------------------------------------------
																...
																// Traffic
																$traffic_list = $modelo->get_traffic_list($modelo->getIdPEC(), "", iif($value["ID_TIPO_DET"]), iif($value["ID_TIPO_UTILIZACAO"]));
															...
															// ---------------------------------------------------------------------------
																...
																// Quantity
																$sms_count = $modelo->get_sms_count($modelo->getIdPEC(), iif($value["ID_PEC_TIPO_LIGACAO"]), "", iif($value["ID_TIPO_UTILIZACAO"]));
														...
														// Value list
														$value_list = $modelo->get_value_list($modelo->getIdPEC(), iif($value["ID_PEC_TIPO_LIGACAO"]), "", iif($value["ID_TIPO_UTILIZACAO"]));

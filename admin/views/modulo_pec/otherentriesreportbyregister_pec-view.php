					...
					<?php
						...
						$data_value = $modelo->get_otherentries_detail($modelo->getIdPEC(), $lancamento_id, true);
					?>
														...
														<?php
															...
															foreach ( $data_value as $value )
															{
																...
																if ( isset($value["DATA_PAGAMENTO"]) && $value["DATA_PAGAMENTO"] != "" )
																	echo "<td style='text-align: center;' title='" . $value["DATA_PAGAMENTO"] . "' >" . $value["DATA_PAGAMENTO"] . "</td>";
																else
																	echo "<td style='text-align: center;'> - </td>";

																// Credit date
																if ( isset($value["DATA_CREDITO"]) && $value["DATA_CREDITO"] != "" )
																	echo "<td style='text-align: center;' title='" . $value["DATA_CREDITO"] . "' >" . $value["DATA_CREDITO"] . "</td>";
																else
																	echo "<td style='text-align: center;'> - </td>";
																...
																if ( isset($value["UTILIZADO"]) && $value["UTILIZADO"] != "" )
																{
																	...
																	// Check the time format (hh:mm:ss)
																	if ( strlen($value["UTILIZADO"]) == 8 )
																	{
																		echo "<td style='text-align: center;' title='" . format_mm_ss(format_hh_mm_ss($value["UTILIZADO"])) . "'>" . format_mm_ss(format_hh_mm_ss($value["UTILIZADO"])) . "</td>";
																		$value["UTILIZADO"] = format_mm_ss(format_hh_mm_ss($value["UTILIZADO"]));
																	}
																	else if ( strpos($value["UTILIZADO"], ",") !== false )
																	{
																		$value["UTILIZADO"] = str_replace("m", "", $value["UTILIZADO"]);
																		$value["UTILIZADO"] = str_replace(",", "m", $value["UTILIZADO"]);
																		$value["UTILIZADO"] .= "s";
																	...
																	{
																		//! Add the seconds to the utilization
																		if ( strpos($value["UTILIZADO"], "s") === false && $value["UTILIZADO"] != "" && $value["UTILIZADO"] != "-" )
																		{
																			$value["UTILIZADO"] .= "00s";
																		}
																	...
																	if ( $value["UTILIZADO"] != "" && $value["UTILIZADO"] != "-" )
																	{
																		if ( strpos($value["UTILIZADO"], "m") === false )
																		{
																			// Total duration
																			if ( $count != 0 )
																				$total_time = min_sec_sum($total_time, $value["UTILIZADO"]);
																			else
																				$total_time = $value["UTILIZADO"];
																		}
																		else
																		{
																			// Total duration
																			if ( $count != 0 )
																				$total_time = min_sec_sum($total_time, format_min_sec($value["UTILIZADO"]));
																			else
																				$total_time = format_min_sec($value["UTILIZADO"]);
																			$total_time = str_replace(".", "", $total_time);
																			$show_minutes = 1;
																		}
																	}
																...
																if ( isset($value["SMS"]) && $value["SMS"] != "" )
																{
																	...
																	$value["SMS"] = str_replace(".", "", $value["SMS"]);
																	echo "<td style='text-align: center;' title='" . $value["SMS"] . "'>" . $value["SMS"] . "</td>";
																	$total_sms = real_sum($total_sms, $value["SMS"]);
																}
																else
																	echo "<td style='text-align: center;'>-</td>";

							...
							<div class="page-header">
								<h1 title="Editar Plataforma LD">Editar Plataforma LD <small>edição </small></h1>
							</div>
							<!-- end: PAGE TITLE -->
						</div>
					</div>
					...
					<?php
						...
						if ( isset($_GET['ID_platformLD']) && $_GET['ID_platformLD'] != '' )
						{
							...
							$platformLD_info = $modelo->load_platformLD_info($platformLD_id);

							// Load edit method
							$modelo->edit_tax( $platformLD_id );
									...
									<form action="#" role="form" class="smart-wizard form-horizontal" id="frmCadPlataformaLD" method="POST" enctype="multipart/form-data">
										<div id="wizardPlataformaLD" class="swMain">
											<ul>
												<li>
													<a href="#step-1">
														...
														<small>Passo 1 - editar plataforma LD</small> </span>
													</a>
												</li>
												<li>
													<a href="#step-2">
											...
											<div id="step-1">
												...
												<p>
													Preencha as informações do formulário abaixo para editar a plataforma LD.
												</p>
												<hr>
												<div class="row">
													<div class="col-md-12">
														...
														<div class="col-sm-3">
															...
															sendRequest( '<?php echo HOME_URI;?>/modulo_operadora/cadastrar_plataformaLD', 'action=update_plataforma_LD&carrier=' + this.value, 'POST', '///', document.getElementById('ID_DEGRAU_LD'), 'fill select' );">
																...
																<?php 
																	...
																	{
																		if ( $platformLD_info[0]["ID_OPERADORA"] == $value[0] )
																		{
													...
													<div class="form-group">
														...
														<div class="col-sm-3">
															<select name="ID_DEGRAU_LD" id="ID_DEGRAU_LD" class="form-control search-select" onChange="changeDegrau(this.options[this.selectedIndex].innerHTML);">
																...
																<?php
																	// Print the carrier list
																	if ( isset($platformLD_info[0]["ID_OPERADORA"]) && $platformLD_info[0]["ID_OPERADORA"] != "" )
																	{
																		// Print the plan list
																		$lista = $modelo->get_plataformaLD_list_DDL_array( $platformLD_info[0]["ID_OPERADORA"] );

																		foreach ( $lista as $value )
																		{
																			if ( $platformLD_info[0]["ID_DEGRAU_LD"] == $value[0] )
																			{
																				echo "<option value='" . $value[0] . "' selected>" . $value[1] . "</option>";
																				$plano = $value[1];
																			}
												...
												<div class="form-group">
													<div class="col-sm-3 col-sm-offset-0">
														<button class="btn btn-blue next-step btn-block">
															Editar plataforma <i class="fa fa-arrow-circle-right"></i>
														</button>
													</div>
												</div>
														...
														<div class="col-sm-3">
															<select name="ID_TIPO_TARIFA" id="ID_TIPO_TARIFA" class="form-control search-select" onChange="setTableColumns( this.options[this.selectedIndex].text );" >
																...
																<?php 
																	...
																	foreach ( $lista as $value )
																	{
																		if ( $platformLD_tax_info[0]["ID_TIPO_TARIFA"] == $value[0] )
																		{
																			echo "<option value='" . $value[0] . "' selected>" . $value[1] . "</option>";
																			$tipo_tarifa = $value[1];
																		}
														...
														<div class="col-sm-3">
															<select name="ID_SUBTIPO_TARIFA" id="ID_SUBTIPO_TARIFA" class="form-control search-select" >
																...
																<?php 
																	...
																	foreach ( $lista as $value )
																	{
																		if ( $platformLD_tax_info[0]["ID_SUBTIPO_TARIFA"] == $value[0] )
																			echo "<option value='" . $value[0] . "' selected>" . $value[1] . "</option>";
																		else
																			echo "<option value='" . $value[0] . "'>" . $value[1] . "</option>";
																	}
																	...
																	<tbody>
																		<?php
																			...
																			foreach ( $state_list as $s_list )
																			{
																				...
																				{
																					if ( isset($tax_list_array[$i]) )
																						echo '<td style="text-align: center;"><input type="text" name="TARIFA_ESTADO" class="form-control" style="height: 26px;" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="10" onBlur="getProvinceTaxes( );" value="' . $tax_list_array[$i] . '"/></td>';
																					else
											...
											<?php
												// Load insert method
												$modelo->edit_state_tax( $platformLD_id );
											?>
		...
		<script>getProvinceTaxes( );</script>